...
pub trait CrabyTestSpec {
    ...
    fn emit(&self, signal_name: CrabyTestSignal) {
        ...
        match signal_name {
            CrabyTestSignal::OnFinished => manager.emit(self.id(), "onFinished"),
    ...
    fn emit_array_number(&self, signal_name: CrabyTestSignal, arr: &[f64]) {
        let manager = crate::ffi::bridging::get_signal_manager();
        match signal_name {
            CrabyTestSignal::OnFinished => manager.emit_array_number(self.id(), "onFinished", arr),
    ...
    fn emit_array_string(&self, signal_name: CrabyTestSignal, arr: &[String]) {
        let manager = crate::ffi::bridging::get_signal_manager();
        match signal_name {
            CrabyTestSignal::OnFinished => { let str_refs: Vec<&str> = arr.iter().map(|s| s.as_str()).collect(); manager.emit_array_string(self.id(), "onFinished", &str_refs); },
    ...
    fn emit_array_object(&self, signal_name: CrabyTestSignal, arr: &[String]) {
        let manager = crate::ffi::bridging::get_signal_manager();
        match signal_name {
            CrabyTestSignal::OnFinished => { let str_refs: Vec<&str> = arr.iter().map(|s| s.as_str()).collect(); manager.emit_array_object(self.id(), "onFinished", &str_refs); },
            CrabyTestSignal::OnSignal => { let str_refs: Vec<&str> = arr.iter().map(|s| s.as_str()).collect(); manager.emit_array_object(self.id(), "onSignal", &str_refs); },
        }
    }

    fn emit_object(&self, signal_name: CrabyTestSignal, data: &[u8]) {
        let manager = crate::ffi::bridging::get_signal_manager();
        match signal_name {
            CrabyTestSignal::OnFinished => manager.emit_object(self.id(), "onFinished", data),
            CrabyTestSignal::OnSignal => manager.emit_object(self.id(), "onSignal", data),
        }
    }
